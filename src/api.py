...
async def lifespan(app: FastAPI):
    ...
    # Map to enum
    face_method = getattr(ReplacementMethod, default_face_method, ReplacementMethod.BLUR)
    text_method = getattr(ReplacementMethod, default_text_method, ReplacementMethod.MASK)
...
async def anonymize_image(request: AnonymizationRequest) -> AnonymizationResult:
    ...
    """
    try:
        logger.info(f"Anonymizing image {request.image_id} with {len(request.replacements)} replacements")
        ...
        raise HTTPException(status_code=500, detail=f"Error anonymizing image: {str(e)}")
...
async def download_anonymized_image(
    image_id: str,
    format: str = "png"
) -> Response:
    """
